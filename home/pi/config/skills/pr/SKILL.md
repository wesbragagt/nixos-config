---
name: pr
description: Create or update a GitHub pull request for the current branch. Invokes /commit first to stage and commit any uncommitted changes, then uses the GitHub CLI (gh) to push the branch and either open a new PR or update the existing PR body for the current branch. Use this skill whenever the user wants to open a PR, update an existing PR with new changes, submit their work for review, push a branch and create a pull request, or says anything like "open a PR", "create a pull request", "update the PR", "submit for review", "push and PR", or "ship this".
argument-hint: "[optional PR title or description context]"
---

# pr

Create or update a GitHub pull request from the current branch. Always commit first, then push, then create the PR or update the existing one.
...
- Push the branch so the PR includes the latest commits
