---
name: commit
description: Commit only the changes relevant to the current task. Never uses git add -A or git add . — always stages files selectively.
tools: bash
---
...
Add only the relevant files by name: `git add <file1> <file2> ...`
...
Otherwise, write a concise conventional commit message: `<type>(<optional scope>): <short imperative summary>`
...
Run `git commit -m "<message>"`, then confirm success with `git log --oneline -1`.
...
- Do not override the git author. If the author is wrong, inform the user and ask them to fix their git config before proceeding.
